...
export const addCreditCardMutation = gqlV2/* GraphQL */ `
  mutation AddCreditCardRecurringContributions(
    ...
    addCreditCard(creditCardInfo: $creditCardInfo, name: $name, account: $account, isSavedForLater: $isSavedForLater) {
...
export const confirmCreditCardMutation = gqlV2/* GraphQL */ `
  mutation ConfirmCreditCardRecurringContributions(
    $paymentMethod: PaymentMethodReferenceInput!
    $shouldBeSaved: Boolean
  ) {
    confirmCreditCard(paymentMethod: $paymentMethod, shouldBeSaved: $shouldBeSaved) {
      ...paymentMethodResponseFragment
    }
  }
  ${paymentMethodResponseFragment}
...
const UpdatePaymentMethodPopUp = ({
  ...
  const handleAddPaymentMethodResponse = async response => {
    const { paymentMethod, stripeError, shouldBeSaved } = response;
    if (stripeError) {
      return handleStripeError(paymentMethod, stripeError, shouldBeSaved);
    } else {
      return handleSuccess(paymentMethod);
    }
  };

  const handleStripeError = async (paymentMethod, stripeError, shouldBeSaved) => {
    ...
    if (result.error) {
      ...
      try {
        const response = await submitConfirmPaymentMethodMutation({
          variables: { paymentMethod: { id: paymentMethod.id }, shouldBeSaved },
        });

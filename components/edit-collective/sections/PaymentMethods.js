...
class EditPaymentMethods extends React.Component {
  ...
  submitNewCreditCard = async () => {
    ...
    if (!data || !this.state.stripe) {
      ...
      try {
        ...
        const { paymentMethod, stripeError, shouldBeSaved } = res.data.addCreditCard;

        if (stripeError) {
          this.handleStripeError(paymentMethod, stripeError, shouldBeSaved);
        } else {
          this.handleSuccess();
        }
      } catch (e) {
  ...
  handleStripeError = async (paymentMethod, stripeError, shouldBeSaved) => {
